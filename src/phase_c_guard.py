...
def pii_scan(text: str, analyzer=None, anonymizer=None) -> dict:
    ...
    allowed_entities = ["VN_CCCD", "VN_PHONE", "EMAIL_ADDRESS"]
    results = analyzer.analyze(text=text, language=PRESIDIO_LANGUAGE, entities=allowed_entities, score_threshold=0.7)
    if not results:
...
async def check_input_rail(text: str, rails=None) -> dict:
    ...
    # Fallback guardrail due to Groq model silent failures on intent matching
    if not blocked:
        adv_kws = ["bỏ qua", "ignore", "pretend", "override", "đóng vai", "không có giới hạn", "thơ", "nấu ăn", "cổ phiếu", "bitcoin", "recommend", "giải toán", "tiết lộ", "dump", "forget"]
        if any(kw in text.lower() for kw in adv_kws):
            blocked = True
