...
from config import GROQ_API_KEY, GROQ_BASE_URL, JUDGE_MODEL, HUMAN_LABELS_PATH
...
Chỉ trả lời đúng định dạng JSON sau, không có text nào khác:
{{"winner": "A", "reasoning": "ly do", "scores": {{"A": 0.8, "B": 0.6}}}}

winner phải là "A" hoặc "B" hoặc "tie". scores là số từ 0.0 đến 1.0.
'''
    ...
    # Retry tối đa 3 lần (Groq đôi khi trả JSON rỗng)
    for attempt in range(3):
        try:
            resp = client.chat.completions.create(
                model=JUDGE_MODEL,
                messages=[
                    {"role": "system", "content": "Bạn là expert đánh giá RAG. Luôn trả lời JSON hợp lệ."},
                    {"role": "user",   "content": prompt_text},
                ],
                temperature=0.1,
            )
            raw = resp.choices[0].message.content or ""
            # Tìm JSON object trong response (kể cả khi có text thừa)
            match = re.search(r'\{.*\}', raw, re.DOTALL)
            if match:
                result = json.loads(match.group())
                result.setdefault("winner", "tie")
                result.setdefault("reasoning", raw[:200])
            ...
            if attempt == 2:
                break  # Trả về default sau 3 lần thất bại

    return {"winner": "tie", "reasoning": "Parse error after retries", "scores": {"A": 0.5, "B": 0.5}}
